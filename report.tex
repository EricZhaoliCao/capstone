...
The results above should be interpreted with three caveats.  First, the
...
significantly worse than the bid--ask mid, and order-book depth has not
been analysed.  Second, the optimal decision window (3 minutes) was
validated only on KAITO over a four-month test period, so cross-asset and
longer-horizon robustness are untested.  Third, the strategy is
intentionally simple---fully rule-based, without predictive modelling---and
serves as a transparent baseline against which more sophisticated
approaches can be benchmarked.
