...
$2.35$ but is fragile to slippage and timing.  \textbf{Strategy~2} is a
cost-aware positive funding-basis carry on Binance (BTC/ETH/SOL) that,
after real spread calibration, reaches a normalised net PnL of $0.0462$
(Sharpe $7.65$, max drawdown $-0.11\%$) by trading only the
positive-funding, positive-basis subset.  \textbf{Strategy~3} shows that
the na\"ive funding-rank cross-section \emph{loses} money on full PnL
(Sharpe $-2.54$) and builds two delta-neutral trades instead: a scalable
cross-exchange spread (Strategy~A, Sharpe $2.34$) and a negative
cash-and-carry (Strategy~B, in-regime $7.47$, honestly de-risked to
$\approx 3$ and capped at \$130{,}926).  Across all three, the common
lesson is the same: funding carry is real but small, survives only when
price risk and execution cost are accounted for honestly, and is bounded
by capacity and regime rather than by signal quality.
\end{abstract}
...
%% =====================================================================
\section{Strategy 2: A Cost-Aware Positive Funding-Basis Carry}
\label{sec:strat2}
%% =====================================================================
...
%% =====================================================================
\section{Strategy 3: Cross-Exchange Spread Capture and Negative Cash-and-Carry}
\label{sec:strat3}
%% =====================================================================
...
Taken together, the three strategies map the funding-rate arbitrage space
...
direct convergence pull is small (Strategy~2 finds a same-sign basis
...
harvesting fails---Strategy~3 shows a funding-rank cross-section losing
...
\emph{Strategy~2} (Liang Mengxuan) trades a positive funding-\emph{basis}
carry on Binance majors, deliberately restricted to the long-spot /
short-perpetual side so that no unmodelled spot borrowing is required; with
real spread calibration it delivers a stable normalised net PnL of $0.0462$
(Sharpe $7.65$, drawdown $-0.11\%$) and survives train--test and
walk-forward checks.  \emph{Strategy~3} (Cao Zhaoli) trades the same-coin
cross-venue funding spread at scale (Strategy~A, Sharpe $2.34$, any size)
and a single-venue negative cash-and-carry (Strategy~B), whose headline
in-regime $7.47$ is honestly deflated by capacity and regime to a
long-run $\approx 3$ within $[2.41, 3.81]$ and a hard \$130{,}926 cap.
...
% Strategy 2 (Liang Mengxuan) citations
